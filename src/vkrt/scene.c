...
void recordFrameTime(VKRT* vkrt) {
    ...
    uint32_t calculatedSPP = (uint32_t)glm_clamp(vkrt->averageFrametime / (vkrt->renderTimeMs + 0.6f) * vkrt->sceneData->samplesPerPixel, 1, 1024);
    uint32_t currentSPP = vkrt->sceneData->samplesPerPixel;

    if (frameNumber > 4 && frameNumber % 8 == 0) {
